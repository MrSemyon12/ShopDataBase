...
namespace ShopDataBase
{
    public class DictShop
    {
        ...
        public int GetHash(string name, string address)
        {           
            int sum = 0;
        ...
        public Item<string, string> Search(string name, string address)
        {
            var hash = GetHash(name, address);
            compare = 0;
            ...
            if (Items[hash].Key.Equals(name) && Items[hash].Value.Equals(address))
            {
                compare++;
                return Items[hash];
            }
            else
            {
                for (var i = hash; i < size; i++)
                {
                    ...
                    if (Items[i].Key.Equals(name) && Items[i].Value.Equals(address))
                    {
                        return Items[i];
                    }                    
                }
        ...
        public void Add(Item<string, string> item)
        {
            ...
            if (fullness == 3 * size / 4) // Заполненность 75%
            {
                size = size * 2;
                Resize();
            }
        ...
        public void Remove(string name, string address)
        {
            ...
            else
            {
                ...
                if (removed && size > 10 && fullness == size / 4) // Заполненность 25%
                {
                    size = size / 2;
                    Resize();
                }
